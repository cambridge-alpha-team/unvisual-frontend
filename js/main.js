...
Mousetrap.bind([ 'c' ], function() {
	if (activeNode.name == "fx" || activeNode.name == "play"
			|| activeNode.name == "sleep" || activeNode.name == "sample"
					...
					if ([ 'root', 'fx' ].indexOf(activeNode.parent.name) < 0
						...
						if (mode == 'add') {
							if (activeNode.parent.name.substr(0, 4) == 'loop') {
								codeTypes = [ "play", "sleep", "fx", "synth",
										"sample" ];
							} else {
								codeTypes = [ "loop", "play", "sleep", "fx",
										"synth", "sample" ];
							}
							selectedCodeType = 0;
							say("What do you want to add? "
									+ codeTypes[selectedCodeType] + "; "
									+ (selectedCodeType + 1) + " of "
									+ codeTypes.length);
...
Mousetrap
		.bind(
				[ 'minus' ],
				function() {
					if (activeNode.name == "tempo"
							|| (activeNode.parent.children.length == 1 && activeNode.name != 'fx')
							|| activeNode instanceof ChoiceNode
							|| activeNode.parent instanceof PlayNode) {
						say('You cannot delete this code. '
								+ activeNode.readName()
								+ ' is currently selected');
						mode = null;
					} else {
...
Mousetrap
		.bind(
				...
				function() {
					...
					case 'add': // add code
						mode = null;
						say("Adding code cancelled. The currently selected bit of code is "
								+ activeNode.readFull());
						break;
					case 'bind-cubelet': // select cubelet
						mode = null;
						say("Cubelet selection cancelled. The currently selected bit of code is "
								+ activeNode.readFull());
						break;
					case 'delete': // delete
						mode = null;
						say("Delete cancelled. The currently selected bit of code is "
								+ activeNode.readFull());
						break;
				...
				function() {
					...
					case 'add': // add code
						if (activeNode.parent.name.substr(0, 4) == 'loop') {
							...
							case 0: // play
								activeNode = new PlayNode(activeNode.parent,
										(activeNode.parent.children
												.indexOf(activeNode) + 1));
								say("New note created");
								break;
							case 1: // sleep
								activeNode = new SleepNode(activeNode.parent,
										(activeNode.parent.children
												.indexOf(activeNode) + 1));
							...
							case 2: // fx
								...
								break;
							case 3: // synth
								activeNode = new SynthNode(activeNode.parent,
										(activeNode.parent.children
												.indexOf(activeNode) + 1));
								say("New synth created");
								break;
							case 4: // sample
								activeNode = new SampleNode(activeNode.parent,
										(activeNode.parent.children
												.indexOf(activeNode) + 1));
					...
					case 'bind-cubelet': // select cubelet
						...
						break;
					case 'delete': // delete
						// Determine the index of activeNode in the parent's
						// array of children
						var index = activeNode.parent.children
								.indexOf(activeNode);
						if (index >= 0) {
							if (activeNode.name == "fx") {
								// Parent activeNode's children to activeNode's
								// parent
								for ( var i = 1; i < activeNode.children.length; i++) {
									var childNode = activeNode.children[i];
									activeNode.parent.children.splice(
											index + i, 0, childNode);
									childNode.parent = activeNode.parent;
								}
							}
							// Remove activeNode from its parent's list of
							// children
							...
							say("Code deleted. The currently selected bit of code is "
									+ activeNode.readFull());
						} else {
					...
					default:
						if (activeNode.children.length > 0) {
							...
							say("Go into " + oldActive.readName() + ".   "
									+ activeNode.readFull());
						} else if (activeNode instanceof ValueNode
								|| activeNode instanceof ChoiceNode) {
							selectedChoice = activeNode.choices
									.indexOf(activeNode.choice);
							mode = 'choose-value';
							if (activeNode.name == "sleep") {
								say("How many beats do you want to sleep for? Choose a value between 0.125 and 4: "
										+ activeNode.choices[selectedChoice]);
							} else if (activeNode.name == "note") {
								say("Choose a value between 40 and 100 for your note: "
										+ activeNode.choices[selectedChoice]);
							} else if (activeNode.name == "amp") {
								say("How loud do you want this to be? Choose a value between 0 and 1: "
										+ activeNode.choices[selectedChoice]);
							} else if (activeNode.name == "release") {
								say("How slowly do you want to go from full amplitude to silence? Choose a value between 0 and 5: "
										+ activeNode.choices[selectedChoice]);
							} else if (activeNode.name == "tempo") {
								say("Choose a tempo for your piece between 60 and 180: "
										+ activeNode.choices[selectedChoice]);
							} else if (activeNode.name == "synth name") {
								say("Choose a synth: "
										+ activeNode.choices[selectedChoice]);
							} else if (activeNode.name == "fx name") {
								say("Choose an effect: "
										+ activeNode.choices[selectedChoice]);
